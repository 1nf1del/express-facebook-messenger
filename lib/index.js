...
Bot.prototype._handleMessage = function(req, next){
  ...
  for (i = 0; i < events.length; i++) {
    ...
    if (event.message && event.message.text) {
      ...
      debug('got message %s %s', senderId, msg);
...
Bot.prototype.send = function(senderId, msg){
  ...
  debug('sending message to %s %s', senderId, msg);
  ...
  request
    ...
    .end(function(err, response){
      ...
      debug('sent message %s', response.body);
    });
...
Bot.prototype.router = function(){
  ...
  router.get('/', function(req, res, next) {
    if (req.query['hub.verify_token'] !== self._token) next(new Error('wrong validation token'));
